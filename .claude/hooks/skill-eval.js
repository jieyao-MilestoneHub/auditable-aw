#!/usr/bin/env node
/**
 * AAW Skill Evaluation Engine v1.0
 *
 * Intelligent skill activation for Auditable AI Workspace based on:
 * - Keywords and patterns in prompts
 * - File paths mentioned or being edited
 * - Directory mappings
 * - Intent detection
 * - AAW workflow detection
 *
...
function extractFilePaths(prompt) {
  ...
  const extensionPattern =
    /(?:^|\s|["'`])([\w\-./]+\.(?:[tj]sx?|json|ya?ml|md|sh|env|pem|key))\b/gi;
  let match;
  ...
  const dirPattern =
    /(?:^|\s|["'`])((?:packages|tools|\.claude|\.github|\.aaw|\.devcontainer)\/[\w\-./]+)/gi;
...
function evaluate(prompt) {
  ...
  output += 'AAW SKILL ACTIVATION REQUIRED\n\n';
  ...
  output += 'This is an AUDITABLE workspace. Before implementing:\n';
  output += '1. Use /aaw-plan to start a planning session\n';
  output += '2. Use aawctl MCP tools for all operations\n';
  output += '3. Follow aaw-sop and aaw-security skill guidelines\n';
